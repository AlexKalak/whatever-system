...
type PancakeswapV3SwapEvent struct {
	...
	ProtocolFeesToken0 *big.Int       `json:"protocol_fees_token0"`
	ProtocolFeesToken1 *big.Int       `json:"protocol_fees_token1"`
}

func (e *PancakeswapV3SwapEvent) FromLog(lg types.Log) error {
	if len(lg.Topics) < 3 {
		return chainerrors.ErrUnableToParseLog
	}

	abi, err := GetPancakeswapV3EventsABI()
	if err != nil {
		return err
	}
	...
	ProtocolFeesToken0, ok := out[5].(*big.Int)
	if !ok {
		return chainerrors.ErrUnableToParseLog
	}
	ProtocolFeesToken1, ok := out[6].(*big.Int)
	if !ok {
		return chainerrors.ErrUnableToParseLog
	}
...
type PancakeswapV3MintEvent struct {
	Sender    common.Address `json:"sender"`
...
func (e *PancakeswapV3MintEvent) FromLog(lg types.Log) error {
	if len(lg.Topics) < 4 {
		return chainerrors.ErrUnableToParseLog
	}
...
type PancakeswapV3BurnEvent struct {
	Owner     common.Address `json:"owner"`
	TickLower int32          `json:"tick_lower"`
	TickUpper int32          `json:"tick_upper"`
	Amount    *big.Int       `json:"amount"`
...
func (e *PancakeswapV3BurnEvent) FromLog(lg types.Log) error {
	if len(lg.Topics) < 4 {
		return chainerrors.ErrUnableToParseLog
	}
...
func (PancakeswapV3SwapEvent) isChainEventData() {}
func (PancakeswapV3MintEvent) isChainEventData() {}
func (PancakeswapV3BurnEvent) isChainEventData() {}
