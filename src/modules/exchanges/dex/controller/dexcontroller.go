...
func NewDexController(service service.DexService) *DexController {
	return &DexController{service: service}
}
...
func (c *DexController) GetAll(ctx *fiber.Ctx) error {
	dexes, err := c.service.GetAll()
	if err != nil {
		return ctx.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"message": err.Error()})
	}

	return ctx.JSON(dto.ToDexResponses(dexes))
}
